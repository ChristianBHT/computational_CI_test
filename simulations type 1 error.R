...
for (N in c(500, 800, 2000)) {
  ...
  results <- pblapply(cl=cl, 1:no_tests, function(i){
    ...
    Interactiondata$sinZ1 <- sin(Interactiondata$Z1)
    Interactiondata$sinZ2 <- sin(Interactiondata$Z2)
    Interactiondata$cosZ1 <- cos(Interactiondata$Z1)
    Interactiondata$cosZ2 <- cos(Interactiondata$Z2)
    Interactiondata$expZ1 <- exp(Interactiondata$Z1)
    Interactiondata$expZ2 <- exp(Interactiondata$Z2)
    Interactiondata$logZ1 <- exp(abs(Interactiondata$Z1))
    ...
    for (j in 1:R) {
      output[[j]] <- NullGenerator(data = Interactiondata,
                                   formula =  Y ~ X + Z2 + Z2Sqr + Z2Cub + sinZ2 + cosZ2 + expZ2 + logZ2 + Z1 + Z1Sqr + Z1Cub + sinZ1 + cosZ1 + expZ1 + logZ1,
                                   p = p,
    ...
    EpxLogData$sinZ1 <- sin(EpxLogData$Z1)
    EpxLogData$sinZ2 <- sin(EpxLogData$Z2)
    EpxLogData$cosZ1 <- cos(EpxLogData$Z1)
    EpxLogData$cosZ2 <- cos(EpxLogData$Z2)
    EpxLogData$expZ1 <- exp(EpxLogData$Z1)
    EpxLogData$expZ2 <- exp(EpxLogData$Z2)
    EpxLogData$logZ1 <- exp(abs(EpxLogData$Z1))
    ...
    TrigData$sinZ1 <- sin(TrigData$Z1)
    TrigData$sinZ2 <- sin(TrigData$Z2)
    TrigData$cosZ1 <- cos(TrigData$Z1)
    TrigData$cosZ2 <- cos(TrigData$Z2)
    TrigData$expZ1 <- exp(TrigData$Z1)
    TrigData$expZ2 <- exp(TrigData$Z2)
    TrigData$logZ1 <- exp(abs(TrigData$Z1))
    ...
    PolyData$sinZ1 <- sin(PolyData$Z1)
    PolyData$sinZ2 <- sin(PolyData$Z2)
    PolyData$cosZ1 <- cos(PolyData$Z1)
    PolyData$cosZ2 <- cos(PolyData$Z2)
    PolyData$expZ1 <- exp(PolyData$Z1)
    PolyData$expZ2 <- exp(PolyData$Z2)
    PolyData$logZ1 <- exp(abs(PolyData$Z1))
  ...
  # Save output
  filename <- paste0("simulation_type_1_error_", N,".rds")
