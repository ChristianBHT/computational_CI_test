...
for (N in c(500, 800, 2000)) {
  ...
  results <- pblapply(cl=cl, 1:no_tests, function(i){
    ...
    multinom_model <- multinom(Y ~ X + Z2 + Z2Sqr + Z2Cub, data = data)
    ...
    multinom_model <- multinom(Y ~ as.factor(X) + Z2 + Z2Sqr + Z2Cub , data = data)
    model_anova <- Anova(multinom_model, type = "III")
  ...
  # Save output
  filename <- paste0("reject_rate_", N,"_mlog_gcm_kci.rds")
